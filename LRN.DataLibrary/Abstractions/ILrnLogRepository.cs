...
public interface ILrnLogRepository
{
    ...
    Task<LrnStepLog> StartStepAsync(long runId, string labName, int stepSeq, string stepName, string stepCategory, string sourceSystem,
        string? fileIn = null, string? pathIn = null, long? recordsIn = null, CancellationToken ct = default);

    Task CompleteStepAsync(long stepLogId, string status, long? recordsOut, string? fileOut, string? pathOut,
