...
public sealed class SharePointDownloader
{
	...
	{
		var parts = item.Name.Split('.', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);

		if (parts.Length == 0)
			return 0;

		return int.TryParse(parts[0], out var month) ? month : 0;
	}

	private static DateTime DateRangeSortKey(DriveChild item)
	{
		var parts = item.Name.Split(" - ", StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);
