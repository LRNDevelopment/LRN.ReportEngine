...
public sealed class DenialDatabaseWorker : BackgroundService
{
    ...
    private async Task ProcessLabAsync(LabConfig lab, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var claimTable = _excelReader.ReadTable(lab.ClaimActionMapper);
            var mapperIndex = new ClaimActionMapperIndex(claimTable.Rows);
            ...
            var payerTable = _excelReader.ReadTable(lab.PayerPolicyFile);
            ...
            var (headers, finalRows) = _builder.Build(payerTable.Headers, payerTable.Rows, mapperIndex, denialCodeHeader: "DenialCode");
